...
namespace DependencyInjector
{
    public class DI : IDI
    {
        ...
        private readonly ConcurrentDictionary<int, Stack<Type>> recursionControlStackByThreadId
        ...
        public TDependency Resolve<TDependency>(string name = null) where TDependency : class
        {
            ...
            return (TDependency)result;//will cause cast error for generic dependencies wich are not covariant to their generic arguments; but it is considered on registration stage
        }

        internal object Resolve(Type dependency, string name = null)
        {
            CheckRecursionAndTryCreateDependencyIfRecursion(dependency, out Stack<Type> atResolvingTypes, out object result);
            if (result != null)
                return result;
            ...
            if (typeof(IEnumerable).IsAssignableFrom(dependency))
                GetInstanciesForAllRegistratedDependencyImplementations(dependency, name, ref result);
            else
            {
                Implementation[] impls = Configuration.GetImplementations(dependency)?.ToArray();
                ...
                if (impls != null)
                {
                    ...
                    result = implToUse?.ResolveOrReturnSingletonInstance(this, name);
                }
        ...
        private void CheckRecursionAndTryCreateDependencyIfRecursion(
            Type dependency,
            out Stack<Type> atResolvingTypes,
            out object result)
        {
            result = null;
            ...
            if (!recursionControlStackByThreadId
                .TryGetValue(currThreadId, out atResolvingTypes))
            {
        ...
        private void GetInstanciesForAllRegistratedDependencyImplementations(
            Type dependency,
            string implName,
            ref object result)
        {
            ...
            {
                implInstances[i] = impls[i].ResolveOrReturnSingletonInstance(this, implName);
            }
